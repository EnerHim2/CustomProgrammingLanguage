...
class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(pos_start, pos_end, 'ExpectedChar', details)
...
class Lexer:
    ...
    def make_tokens(self):
            ...
            while self.current_char != None:
                if self.current_char in ' \t':
                    ...
                    tokens.append(Token(TT_NEWLINE, TT_NEWLINE, pos_start=self.pos))
                    self.advance()
                    ...
                    tokens.append(Token(TT_PLUS, TT_PLUS, pos_start=self.pos))
                    self.advance()
                elif self.current_char == '-':
                    tokens.append(self.make_minus())
                elif self.current_char == '*':
                    tokens.append(Token(TT_MUL, TT_MUL, pos_start=self.pos))
                    self.advance()
                elif self.current_char == '/':
                    tokens.append(Token(TT_DIV, TT_DIV, pos_start=self.pos))
                    self.advance()
                elif self.current_char == '%':
                    tokens.append(Token(TT_MODULO, TT_MODULO, pos_start=self.pos))
                    self.advance()
                elif self.current_char == '^':
                    tokens.append(Token(TT_POW, TT_POW, pos_start=self.pos))
                    self.advance()
                elif self.current_char == '(':
                    tokens.append(Token(TT_LPAREN, TT_LPAREN, pos_start=self.pos))
                    self.advance()
                elif self.current_char == ')':
                    tokens.append(Token(TT_RPAREN, TT_RPAREN, pos_start=self.pos))
                    self.advance()
                elif self.current_char == '[':
                    tokens.append(Token(TT_LSQUARE, TT_LSQUARE, pos_start=self.pos))
                    self.advance()
                elif self.current_char == ']':
                    tokens.append(Token(TT_RSQUARE, TT_RSQUARE, pos_start=self.pos))
                    self.advance()
                elif self.current_char == "!":
                    ...
                    tokens.append(Token(TT_COMMA, TT_COMMA, pos_start=self.pos))
                    self.advance()
                else:
    ...
    def make_minus(self):
        ...
        return(Token(tok_type, tok_type, pos_start, self.pos))
    ...
    def make_not_equals(self):
        ...
        if self.current_char == "=":
            ...
            return Token(TT_NE, TT_NE, pos_start, self.pos), None
    ...
    def make_equals(self):
        ...
        return Token(tok_type, tok_type, pos_start=pos_start, pos_end=self.pos)
    ...
    def make_less_than(self):
        ...
        return Token(tok_type, tok_type, pos_start, pos_end=self.pos)
...
class Parser:
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expectecd 'return', 'continue', 'break', 'var', 'if', 'for', 'while', 'func', int, float, identifier, '+', '-', '(', '[' or 'not'"))
    ...
    def expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'var', 'if', 'for', 'while', 'func', int, float, identifier, '+', '-', '(', '[' or 'not'"))
    ...
    def comp_expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected int, float, identifier, '+', '-', '(', '[', 'if', 'for', '', 'func' or 'not'"))
    ...
    def call(self):
        res = ParseResult()
        minfac = res.register(self.minfac())
        if res.error: return res
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            return res.success(CallNode(minfac  , arg_nodes))
        return res.success(minfac)
...
class BaseFunction(Value):
    ...
    def generate_new_context(self):
        new_c = Context(self.name, self.context, self.pos_start)
        new_c.symbol_table = SymbolTable(new_c.parent.symbol_table)
        return new_c
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_eval(self, exec_ctx):
        try:
            evaluation = eval(str(exec_ctx.symbol_table.get("expression")))
        except:
            return RTResult().failure(RTError(self.pos_start, self.pos_end, 'Expression is invalid or not supported.', exec_ctx))
...
String.himanshu = String("Made by Himanshu")
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_PLUS:
            ...
            result, error = left.subt_by(right)
...
global_symbol_table.set("himanshu", String.himanshu)
